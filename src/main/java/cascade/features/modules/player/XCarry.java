...
extends Module {
    ...
    public XCarry() {
        super("XCarry", Module.Category.PLAYER, "A fucking x carry dude");
        this.setInstance();
    }
    ...
    public void onDisable() {
        if (!XCarry.fullNullCheck()) {
            if (!this.simpleMode.getValue().booleanValue()) {
                ...
                mc.getConnection().sendPacket((Packet)new CPacketCloseWindow(XCarry.mc.player.inventoryContainer.windowId));
            }
        }
    }
    ...
    private GuiInventory createGuiWrapper(GuiInventory gui) {
        try {
            ...
            ReflectiveOperationException e = ex = new ReflectiveOperationException();
            e.printStackTrace();
            return null;
        }
    }
