        ...
        if (commands.length >= 2) {
            switch (commands[0]) {
                case "add": {
                    ...
                    FriendCommand.sendMessage(ChatFormatting.GREEN + commands[1] + " has been friended", true, false);
                    return;
                }
                case "del": {
                    Cascade.friendManager.removeFriend(commands[1]);
                    FriendCommand.sendMessage(ChatFormatting.RED + commands[1] + " has been unfriended", true, false);
                    return;
                }
            }
